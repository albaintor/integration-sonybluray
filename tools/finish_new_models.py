#!/usr/bin/env python3
"""Apply the remaining new-model support fixes and add protocol regressions."""

from pathlib import Path


def replace_once(path: str, old: str, new: str) -> None:
    """Replace one exact source fragment and fail if the branch diverged."""
    file_path = Path(path)
...
def patch_device() -> None:
    """Fix registration ordering and aiohttp cookie mappings."""
    path = "src/sonyapilib/device.py"
    replace_once(
        path,
        """        self.api_version = 0
...
def patch_client() -> None:
    """Correct decorator typing and make optional device flow explicit."""
    path = "src/client.py"
    replace_once(
        path,
        """def cmd_wrapper(
...
def write_tests() -> None:
    """Write protocol-level tests for the new backends and X700 regression."""
    tests = Path("tests")
    tests.mkdir(exist_ok=True)
    Path("tests/test_protocols.py").write_text(
